const testimonials = {
    fabio: {
        ...
        mediaType: "video",
        mediaUrl: "https://www.youtube.com/embed/pqSPQa-ByKs",
...
function openModal(testimonialId) {
    ...
    try { window.parent.postMessage({ testimonialModalOpen: true }, '*'); } catch (e) {}
...
function closeModal() {
    ...
    // Svuota il contenuto per fermare eventuali video/audio iframe
    setTimeout(() => {
        if (modalContent) modalContent.innerHTML = '';
    }, 300);
    try { window.parent.postMessage({ testimonialModalOpen: false }, '*'); } catch (e) {}
}
